...
\newcommand{\journals}[1]{\FLEX{} (\flex), \INS{} (\ins), \EJOR{} (\ejor), \IJSS{} (\ijss), \MATHJ{} (\mathj), \TETCI{} (\tetci), \CIM{} (\cim), \EXSY{} (\exsy), \EAAI{} (\eaai), \ASC{} (\asc), \PROC{} (\proc), \PLOS{} (\plos), \CIN{} (\cin), \MINE{} (\mine), \FCS{} (\fcs), \ACISC{} (\acisc), \SPJ{} (\spj), \AOR{} (\aor), \JEA{} (\jea), \ITOR{} (\itor), \IJPEDS{} (\ejie), \CAOR{} (\caor), \EJIE{} (\ejie), \ORIJ{} (\orij), \CMPLX{} (\cmplx), \IEI{} (\iei), \IJPEDS{} (\ijpeds), \IJLSM{} (\ijlsm), \JS{} (\js), \KBS{} (\kbs), \ENER{} (\ener), \SWEC{} (\swec), \IJDRR{} (\ijdrr), {#1} \SC{} (\scj).}
