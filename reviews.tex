...
\newcommand{\journals}[1]{\FLEX{} (\flex), \INS{} (\ins), \EJOR{} (\ejor), \IJSS{} (\ijss), \MATHJ{} (\mathj), \TETCI{} (\tetci), \EAAI{} (\eaai), \EXSY{} (\exsy), \CIM{} (\cim), \SWEC{} (\swec), \ASC{} (\asc), \PROC{} (\proc), \PLOS{} (\plos), \ITOR{} (\itor), \KBS{} (\kbs), \CIN{} (\cin), \MINE{} (\mine), \FCS{} (\fcs), \ACISC{} (\acisc), \SPJ{} (\spj), \AOR{} (\aor), \JEA{} (\jea), \CAOR{} (\caor), \ESWA{} (\eswa), \IJPEDS{} (\ejie), \EJIE{} (\ejie), \ORIJ{} (\orij), \CMPLX{} (\cmplx), \IEI{} (\iei), \IJLSM{} (\ijlsm), \IJPEDS{} (\ijpeds), \JS{} (\js), \ENER{} (\ener), \IJDRR{} (\ijdrr), {#1} \SC{} (\scj).}
