...
\newcommand{\EL}{\myurls{https://ietresearch.onlinelibrary.wiley.com/journal/1350911X}{Electronics Letters}}					                                            %% 36
\newcommand{\TVT}{\myurls{https://ieeexplore.ieee.org/xpl/RecentIssue.jsp?punumber=25}{IEEE Transactions on Vehicular Technology}}					                        %% 37
\newcommand{\ACCESS}{\myurls{https://ieeeaccess.ieee.org}{IEEE Access}}					                                                                                    %% 38
...
%% Sum of reviews
%% Papers
\newcommand{\SumPaper}{\the\numexpr \ejor + \ins + \exsy + \iei + \itor + \orij + \ijpeds + \flex + \ejie + \cin + \scj + \cmplx + \fcs + \spj + \jea + \cim + \mine + \acisc + \ijss + \jifs + \ijlsm + \proc + \aor + \tetci + \plos + \js + \mathj + \kbs + \caor + \eaai + \ener + \asc + \swec + \ijdrr + \eswa + \el + \tvt + \access \relax}
%% Conferences
...
\newcommand{\journals}[1]{\INS{} (\ins), \FLEX{} (\flex), \EJOR{} (\ejor), \IJSS{} (\ijss), \EAAI{} (\eaai), \EXSY{} (\exsy), \MATHJ{} (\mathj), \TETCI{} (\tetci), \CIM{} (\cim), \SWEC{} (\swec), \CAOR{} (\caor), \ASC{} (\asc), \PROC{} (\proc), \PLOS{} (\plos), \KBS{} (\kbs), \ESWA{} (\eswa), \ITOR{} (\itor), \CIN{} (\cin), \MINE{} (\mine), \FCS{} (\fcs), \ACISC{} (\acisc), \SPJ{} (\spj), \AOR{} (\aor), \JEA{} (\jea), \IJPEDS{} (\ejie), \EJIE{} (\ejie), \ORIJ{} (\orij), \CMPLX{} (\cmplx), \IEI{} (\iei), \IJLSM{} (\ijlsm), \IJPEDS{} (\ijpeds), \JS{} (\js), \ENER{} (\ener), \IJDRR{} (\ijdrr), \EL{} (\el), \TVT{} (\tvt), \ACCESS{} (\access) {#1} \SC{} (\scj).}
