...
%% Sum of reviews
\newcommand{\Suma}{\the\numexpr \ejor + \ins + \exsy + \iei + \itor + \orij + \ijpeds + \flex + \ejie + \cin + \scj + \cmplx + \fcs + \spj + \jea + \cim + \mine + \acisc + \ijss + \jifs + \ijlsm + \proc + \aor + \tetci + \plos + \js \relax}
...
\newcommand{\journals}[1]{\FLEX{} (\flex), \INS{} (\ins), \EJOR{} (\ejor), \IJSS{} (\ijss), \EXSY{} (\exsy), \PROC{} (\proc), \CIN{} (\cin), \MINE{} (\mine), \FCS{} (\fcs), \ACISC{} (\acisc), \SPJ{} (\spj), \AOR{} (\aor), \TETCI{} (\tetci), \JEA{} (\jea), \IJPEDS{} (\ejie), \EJIE{} (\ejie), \ORIJ{} (\orij), \CMPLX{} (\cmplx), \ITOR{} (\itor), \CIM{} (\cim), \IEI{} (\iei), \IJPEDS{} (\ijpeds), \IJLSM{} (\ijlsm), \PLOS{} (\plos), \JS{} (\js), {#1} \SC{} (\scj).}
