...
\newcommand{\journals}[1]{\FLEX{} (\flex), \INS{} (\ins), \EJOR{} (\ejor), \IJSS{} (\ijss), \EXSY{} (\exsy), \PROC{} (\proc), \CIN{} (\cin), \MINE{} (\mine), \FCS{} (\fcs), \ACISC{} (\acisc), \SPJ{} (\spj), \AOR{} (\aor), \TETCI{} (\tetci), \JEA{} (\jea), \PLOS{} (\plos), \IJPEDS{} (\ejie), \EJIE{} (\ejie), \ORIJ{} (\orij), \CMPLX{} (\cmplx), \ITOR{} (\itor), \CIM{} (\cim), \IEI{} (\iei), \IJPEDS{} (\ijpeds), \IJLSM{} (\ijlsm), \JS{} (\js), {#1} \SC{} (\scj).}
