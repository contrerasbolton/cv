...
\newcommand{\MATHJ}{\myurls{hhttps://www.mdpi.com/journal/mathematics}{Mathematics}}                                                                         %% 27
...
%% Sum of reviews
\newcommand{\Suma}{\the\numexpr \ejor + \ins + \exsy + \iei + \itor + \orij + \ijpeds + \flex + \ejie + \cin + \scj + \cmplx + \fcs + \spj + \jea + \cim + \mine + \acisc + \ijss + \jifs + \ijlsm + \proc + \aor + \tetci + \plos + \js + \mathj \relax}
...
\newcommand{\journals}[1]{\FLEX{} (\flex), \INS{} (\ins), \EJOR{} (\ejor), \IJSS{} (\ijss), \EXSY{} (\exsy), \PROC{} (\proc), \MATHJ{} (\mathj), \TETCI{} (\tetci), \CIN{} (\cin), \MINE{} (\mine), \FCS{} (\fcs), \ACISC{} (\acisc), \SPJ{} (\spj), \AOR{} (\aor), \JEA{} (\jea), \PLOS{} (\plos), \IJPEDS{} (\ejie), \EJIE{} (\ejie), \ORIJ{} (\orij), \CMPLX{} (\cmplx), \ITOR{} (\itor), \CIM{} (\cim), \IEI{} (\iei), \IJPEDS{} (\ijpeds), \IJLSM{} (\ijlsm), \JS{} (\js), {#1} \SC{} (\scj).}
