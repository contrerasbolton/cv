...
\newcommand{\journals}[1]{\FLEX{} (\flex), \INS{} (\ins), \EJOR{} (\ejor), \IJSS{} (\ijss), \TETCI{} (\tetci), \EXSY{} (\exsy), \PROC{} (\proc), \MATHJ{} (\mathj), \PLOS{} (\plos), \CIN{} (\cin), \MINE{} (\mine), \FCS{} (\fcs), \ACISC{} (\acisc), \SPJ{} (\spj), \AOR{} (\aor), \JEA{} (\jea), \ITOR{} (\itor), \EAAI{} (\eaai), \IJPEDS{} (\ejie), \EJIE{} (\ejie), \ORIJ{} (\orij), \CMPLX{} (\cmplx), \CIM{} (\cim), \IEI{} (\iei), \IJPEDS{} (\ijpeds), \IJLSM{} (\ijlsm), \JS{} (\js), \CAOR{} (\caor), \KBS{} (\kbs), {#1} \SC{} (\scj).}
